...
class App:
    ...
    def start_protocol(self):
        self.start_btn.config(state=tk.DISABLED)  # Disable start button to prevent re-starts
        self.status.config(text="Protocol started...")
        Thread(target=self.protocol).start()

    def continue_protocol(self):
        self.continue_btn.config(state=tk.DISABLED)  # Disable continue button after use
